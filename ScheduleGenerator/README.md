...
Generates a full season schedule for an OOTP league, including home/away balancing, mixed series lengths, and calendar-aware spacing to create a realistic season rhythm. The output is intended for use as an OOTP `.lsdl` schedule file.
...
- `-bg, --balanced`: Toggle balanced scheduling behavior (`0` or `1`). This value is written into the generated XML as `balanced_games`.
...
- `-sdw, --start-day-of-week`: Day the schedule should begin on. Accepts names like `Monday` or numeric values `1-7`.
- `-sm, --start-month`: Month the schedule should begin in. Accepts month names like `April` or numeric values `1-12`.
...
- `-ed, --end-divisional`: Number of divisional series to reserve for the very end of the season. This helps lock divisional play to the final stretch.
...
The generator prefers solutions that keep the home/away split balanced and preserve realistic series spacing while maintaining the requested total game count.
...
The script also allows the season to close out with a set number of reserved divisional windows using `-ed`, which helps keep the final stretch geographically and competitively realistic.
